clear
load E_CB.mat                   %换热器P2S，E(NTU,Cr)查表函数，Cr不为1
...
[SDH_R_T_CS,SDH_R_RHL]=CSWDC_SDH_R(T_s,SDH_R_T_in_o,SDH_L,SDH_V_w,SDH_u,SDH_d,SDH_D,SDH_A,SDH_alpha_tt,alpha_w,rou_w,Cp_w,SDH_nx,nt,SDH_dx,dt);
[SDH_S_T_CS,SDH_S_RHL]=CSWDC_SDH_S(T_s,SDH_S_T_in_o,SDH_L,SDH_V_w,SDH_u,SDH_d,SDH_D,SDH_A,SDH_alpha_tt,alpha_w,rou_w,Cp_w,SDH_nx,nt,SDH_dx,dt);
...
[PDH_R_T_CS,PDH_R_RHL]=CSWDC_PDH_R(T_s,PDH_R_T_in_o,PDH_L,PDH_V_w,PDH_u,PDH_d,PDH_D,PDH_A,PDH_alpha_tt,alpha_w,rou_w,Cp_w,PDH_nx,nt,PDH_dx,dt);
[PDH_S_T_CS,PDH_S_RHL]=CSWDC_PDH_S(T_s,PDH_S_T_in_o,PDH_L,PDH_V_w,PDH_u,PDH_d,PDH_D,PDH_A,PDH_alpha_tt,alpha_w,rou_w,Cp_w,PDH_nx,nt,PDH_dx,dt);
...
if  Cr_P2S(1)==1               %判断热容率是否为1
    syms NTU_cal
    E_P2SS(NTU_cal)=NTU_cal/(NTU_cal+1);        %热容率为1时的E(NTU)
...
%      Nu_S2R(i)=((f(i)/8)*(Re(i)-1000)*Pr(i))/(1+12.7(f(i)/8*(Pr(i)^(2/3)-1)^0.5));                    %Nusselt数
 ...
 eval(['save ' '环境温度(加100后为)' num2str(T_s(wxh)+100) '.mat'])
end
