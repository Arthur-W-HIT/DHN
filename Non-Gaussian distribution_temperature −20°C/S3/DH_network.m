...
SDH_S_T_all = zeros(SDH_nx, nt);              %二级热网温度场（x，t）
...
%      Nu_S2R(i)=((f(i)/8)*(Re(i)-1000)*Pr(i))/(1+12.7(f(i)/8*(Pr(i)^(2/3)-1)^0.5));                    %Nusselt数
     ...
     Q_S2R_known=Q_R2SURR(i);
     SDH_u_guess = SDH_u(i);
     tolerance = 1;
  for iter = 1:1000
    % 检查猜测值是否为正数
    if SDH_u_guess <= 0
        error('SDH_u_guess should be positive.')
    end
    
    Re(i) = rou_w * SDH_u_guess * SDH_d / miu_w;
    
    % 检查Re(i)是否为正数，避免log(Re(i))出现复数
    if Re(i) <= 0
        error('Re(i) should be positive.')
    end
    
    Pr(i) = Cp_w * miu_w / k_w;
    f(i) = (0.790 * log(Re(i)) - 1.64)^(-2);  % 计算摩擦系数
    
    Nu_S2R(i) = (f(i)/8 * (Re(i) - 1000) * Pr(i)) / ...
                (1 + 12.7 * (f(i)/8)^0.5 * (Pr(i)^(2/3) - 1));
    
    % 检查Nu_S2R(i)是否为非零数
    if isnan(Nu_S2R(i)) || Nu_S2R(i) == 0
        error('Nu_S2R(i) is invalid.')
    end
    
    h_S2R(i) = k_w * SDH_d * Nu_S2R(i);
    
    % 检查h_S2R(1)是否为非零数，避免除以零
    if h_S2R(1) == 0
        error('h_S2R(1) is zero, check input values.')
    end
    
    % 计算Q_S2R并比较与已知的Q_S2R_known
    Q_S2R_calculated = h_S2R(i) / h_S2R(1) * ...
        (SDH_S_T(SDH_nx) - R_T) / (SDH_S_T_CS(SDH_nx) - R_T_o) * Q_S2R_o;
    
    % 检查Q_S2R_calculated是否为非零值
    if isnan(Q_S2R_calculated) || Q_S2R_calculated == 0
        error('Q_S2R_calculated is invalid.')
    end
    
    % 误差检查
    error = abs(Q_S2R_calculated - Q_S2R_known);
    
    if error < tolerance
        break;  % 若误差在容差范围内，终止迭代
    end
    
    % 更新SDH_u_guess的值
    SDH_u_guess = SDH_u_guess * (Q_S2R_known / Q_S2R_calculated);  % 调整猜测值
end

if  SDH_u_guess>=0&&SDH_u_guess<0.4315*4
    SDH_u(i+1)=SDH_u_guess;
else
    SDH_u(i+1)=SDH_u(i);
end
